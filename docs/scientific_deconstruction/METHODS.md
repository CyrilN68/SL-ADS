...
| 7 | `eval_qualify`   | `sl_ads.evaluate.evaluate_qualify_sbn`              | Per-attack DR / QP / F1 / F2 / TTQ; novelty AUC-ROC. |
...
| **Location** | `src/sl_ads/train/train_models.py:1472–1577` (training); `src/sl_ads/train/compute_evidence.py:267–276` (inference) |
...
| **Mathematical form** | `y_t = g(t) + s(t) + h(t) + ε_t`, with `g(t)` constant (`growth='flat'`, line 1490), `s(t)` weekly + conditional daily-weekday + conditional daily-weekend + hourly seasonalities (lines 1493–1504), `h(t)` from `holidays_df`. Mode `additive` for metrics in `SEASONALITY_ADDITIVE`, else `multiplicative`. |
...
| **R² estimation** | Time-series rolling-origin cross-validation via `prophet.diagnostics.cross_validation(initial='14 days', period='3 days', horizon='1 day')`, then `R²_CV = 1 − MSE/var(y)` (lines 1520–1526). Falls back to in-sample R² with `UserWarning` if CV fails. |
...
| **Location** | `src/sl_ads/train/train_models.py:1266–1450` |
...
| **R² estimation** | 5-fold `TimeSeriesSplit` CV with `r2_score`; `r2 = mean(scores)` (lines 1309–1316). |
| **Mean-fallback** | If `r2_CV < 0` AND rule has `allow_mean_fallback=True` → `DummyRegressor(strategy='mean')`, `r2 ← 0` (lines 1338–1361). Recorded in `_FALLBACK_LOG['reconstruction_dummy']`. Justification: Hyndman & Athanasopoulos (2021) §3.1 baseline. |
| **Why QR not RANSAC** | Documented in lines 1238–1265: training set is by construction attack-free (split before injections), so RANSAC's outlier-rejection causes *seed variance* (22–75 % on `T_susp`); QR(0.5) uses all clean points, deterministic. |
...
| **Location** | `src/sl_ads/train/compute_evidence.py:323–338` |
...
| **Location** | `src/sl_ads/train/train_models.py:160–461` (functions `_grimshaw_fit`, `_evt_threshold`, `_evt_threshold_pair`, `calibrate_thresholds_v2`) |
...
| **Location** | `subjective_logic.py:43–121` |
| **Form** | enforces `Σb + u = 1` (renormalisation if drift > 1e-6, ValueError beyond 1e-6 of consistency error). |
| **Methods** | `projected_prob() = b + a·u` (Jøsang 2016 Eq. 3.23, line 73); `confidence() = 1 − u` (Eq. 3.43, line 76); `uncertainty_maximized()` (Eq. 3.27, lines 80–118). |
...
| **Location** | `subjective_logic.py:128–199` |
...
| **Location** | `subjective_logic.py:80–118` |
...
| **Location** | `compute_evidence.py:38–96`; mirrored single-point version `_apply_trapezoid_single` in `train_models.py:661–680`. |
...
| **Window aggregation** | `r = Σ_{j=1..n_window} (p_j, s_j, n_j)` ⇒ `r_safe + r_susp + r_atk = n_window` (invariant explicit in `compute_evidence.py:74`). |
...
| **Reference** | None canonical; engineering choice. The 5-state directional refinement (`_S_pos`, `_S_neg`, `_N_pos`, `_N_neg`) matches Jøsang 2016 §3.5.4 *coarsening* identity (`r_susp = r_susp_pos + r_susp_neg`, line 70 of `compute_evidence.py:319–322`). |
...
| **Location** | `subjective_logic.py:417–498` |
...
|---|---|---|---|
| asymmetric escalation (default) | `belief_mass` | `compute_asymmetric_escalation_conflict` (`subjective_logic.py:246–306`) | `K = b_prev[safe]·b_curr[atk] + b_prev[atk]·b_curr[safe] + b_prev[safe]·b_curr[susp] + b_prev[susp]·b_curr[atk]` (omits de-escalation cross-products by design) |
| symmetric BCF (canonical) | (used only by `compute_conflict_degree_canonical`) | `subjective_logic.py:309–344` | `K = Σ_{i≠j} b_prev[i]·b_curr[j]` (Jøsang 2016 Eq. 12.4 multinomial extension) |
| projected probability | `projected_prob` | `compute_conflict_degree_projected` (`subjective_logic.py:367–392`) | same cross-products on `P = b + a·u` |
| symmetric KL | `kl_symmetric` | `compute_conflict_degree_kl` (`subjective_logic.py:395–414`) | `K = 1 − exp(−KL_sym(P‖Q) / τ)` with `τ=CONFIG['CONFLICT_KL_TAU']=1.0` |
...
| **Location** | `subjective_logic.py:505–595` |
...
| **Location** | `subjective_logic.py:611–729` |
...
| **Location** | `subjective_logic.py:736–818` |
...
| **Use sites** | (i) Inter-method fusion when `CONFIG['INTER_METHOD_FUSION']='cbf'` (`opinions_pipeline.py:800`), with optional `BALANCE_RATIO` rebalancing. |
...
| **Location** | `subjective_logic.py:924–945` |
| **Form** | `b' = t·b`, `u' = 1 − t·(1−u) = 1 − t·Σb`. |
| **Use site** | optional in WBF mode `trust_discount` (deprecated, retained for ablation only — emits `RuntimeWarning` at import time, `opinions_pipeline.py:143–152`). |
...
| **Location** | `subjective_logic.py:948–987` |
...
| **Location** | `subjective_logic.py:825–853` |
...
| **Documented status** | Lines 345–371 of `opinions_pipeline.py` explicitly label this an *engineering extension* of CBF and recommend the `hierarchical` (0.5/0.5 WBF) variant as the theoretically clean alternative. |
...
| **Location** | `train_models.py:683–809` |
...
| **Use site** | per-metric base rate `a_inj` injected into the bijection at every window in `opinions_pipeline.py:643–648`. Initial state memory `R_init = a_edp · W` (`opinions_pipeline.py:519–527`) so that `proj_atk_init = a_edp[atk]` exactly. |
...
| **Location** | `src/sl_ads/qualify/sbn_qualifier.py` (1826 lines) |
...
| **L3 — template score** | `score(k, g) = Σ_s P^obs_g(s) · c^{k\|g}(s)` where `c^{k\|g}` is an expert-elicited conditional opinion `P(state \| attack_k)` (manually specified, lines 138–511 of `sbn_qualifier.py`). Two modes: `absolute` (default) and `contrastive` (subtracts per-group mean). |
...
| **Justification** | Smoother and unimodal compared to `b_atk` alone (which is bimodal under sparse evidence); see lines 953–959 of `train_models.py`. |
...
| **Location** | `train_models.py:976–1080` |
...
| **Persistence** | Stored both in `models_pkg['_decision_threshold']` and in a sidecar `<model_name>_threshold.json` so downstream code can read it without unpickling the full PKL (`paths.py:36–66`). |
...
| **Location** | `train_models.py:831–951` |
...
|---|---|---|
| `DR` (detection rate, recall) | `n_detected / n_attack_windows` | `evaluate_qualify_sbn.py:457` |
| `QP` (qualification precision) | `n_correct / n_qualified` (top-1 attack-type matches expected) | `evaluate_qualify_sbn.py:518` |
| `F1` | `2·DR·QP / (DR+QP)` | `evaluate_qualify_sbn.py:519` |
| `F_β=2` | `(1+β²)·DR·QP / (β²·DR + QP)` (Tavallaee 2009 IDS standard) | `evaluate_qualify_sbn.py:522` |
| `FAR` (FPR on normal) | `FP / (FP + TN)` excluding `NETWORK_OUTAGE` periods | `evaluate_qualify_sbn.py:204` |
| `MCC` (Matthews) | `(TP·TN − FP·FN)/√{(TP+FP)(TP+FN)(TN+FP)(TN+FN)}` | `evaluate_qualify_sbn.py:205-207` |
| `TTQ` (time-to-qualify) | minutes from `t_start` to first correct top-1 | `evaluate_qualify_sbn.py:534` |

### 4.2 Range-aware AUC / VUS (Paparrizos et al. 2022)

| metric | definition | location |
|---|---|---|
| `R-AUC-ROC(L)` | AUC after expanding each anomaly run by buffer L | `vus_metrics.py:139–155` |
| `R-AUC-PR(L)` | range-aware average precision | `vus_metrics.py:158–172` |
| `VUS-ROC` | `(1/L_max) ∫₀^{L_max} R-AUC-ROC(L) dL` (trapezoidal) | `vus_metrics.py:214–240` |
| `VUS-PR` | analogous on PR | `vus_metrics.py:243–252` |
| Existence recall | fraction of true ranges with ≥ 1 positive prediction | `vus_metrics.py:258–276` |

### 4.3 Axelsson PPV analysis (axelsson_ppv.py)

| metric | definition | location |
|---|---|---|
| Wilson score CI | closed-form for proportions; preferred over Wald near 0/1 | `evaluate/axelsson_ppv.py:197-223` |
...
|---|---|---|
| BCa bootstrap CI | bias-corrected accelerated CI (Efron 1987); jackknife-based acceleration; default `n_boot=2000` | `stats/bootstrap_ci.py:128–150` |
| McNemar test | `χ²` continuity-corrected for `n_disc ≥ 25`, exact binomial for `n_disc < 25` | `stats/mcnemar.py:57–150` |
| Effective sample size | Newey-West-style autocorrelation correction (lag ≤ 10, weights `1 − lag/(L+1)`) | `stats/residual_correlation.py:121–136` |
...
| **Location** | `src/sl_ads/inject/evidence_level.py` (1255 lines) |
...
| **Catalog** | 13 attacks defined in `CONFIG['INJECTED_ATTACK_CATALOG']` (PATCH-C1 unified source, `config.py:1018–1061`): UDP_FLOOD_DDOS, SYN_FLOOD_DDOS, ICMP_FLOOD_BURST, NTP_AMPLIFICATION, DNS_AMPLIFICATION, HTTP_FLOOD_L7_DDOS, SLOWLORIS_DOS, PORT_SCAN, DATA_EXFILTRATION_SLOW, BRUTE_FORCE_SSH, BOTNET_CC_BEACONING, DNS_TUNNELING, UNKNOWN_ANOMALY_CONTROL. |
...
| **Disjointness** | overlap check (`evidence_level.py:876–893`) prevents double-injection. |
